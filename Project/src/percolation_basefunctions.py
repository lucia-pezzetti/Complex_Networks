...
def compute_ranking_DCI_betweenness_centrality(multiplex_structure, N):
    total_nodes = [i for i in range(N)]
    fitness = dict.fromkeys(total_nodes, 0)
    total_degree, participation_degree, dgr_part, degree_layers_multiplex = computing_total_degree_and_participation(multiplex_structure, N)
        ...
        for nodei in multiplex_structure[layerID]:
            for neigh_of_i in multiplex_structure[layerID][nodei]:
                if degree_layers_multiplex[layerID][neigh_of_i] != 0:
                    ...
                    fitness[nodei] += betweenness_centrality_multiplex[(layerID+1)%2][neigh_of_i]
    
    for nodei in fitness:
        if betweenness_centrality_aggregate[nodei] != 0:
            # first term of the product of the generalised DCI
            numerator1 = (betweenness_centrality_multiplex[0][nodei] + 1) * (betweenness_centrality_multiplex[1][nodei] + 1)
            numerator2 = betweenness_centrality_intersection[nodei] + 1
            numerator = numerator1 - numerator2
            denominator = betweenness_centrality_aggregate[nodei] + 1
...
def compute_ranking_DCI_katz_centrality(multiplex_structure, N, alpha=0.07):
    total_nodes = [i for i in range(N)]
    fitness = dict.fromkeys(total_nodes, 0)
    total_degree, participation_degree, dgr_part, degree_layers_multiplex = computing_total_degree_and_participation(multiplex_structure, N)
        ...
        for nodei in multiplex_structure[layerID]:
            for neigh_of_i in multiplex_structure[layerID][nodei]:
                if degree_layers_multiplex[layerID][neigh_of_i] != 0:
                    ...
                    fitness[nodei] += katz_centrality_multiplex[(layerID+1)%2][neigh_of_i] 
    
    for nodei in fitness:
        if katz_centrality_aggregate[nodei] != 0:
            # first term of the product of the generalised DCI
            numerator1 = (katz_centrality_multiplex[0][nodei] + 1) * (katz_centrality_multiplex[1][nodei] + 1)
            numerator2 = katz_centrality_intersection[nodei] + 1 
            numerator = numerator1 - numerator2
            denominator = katz_centrality_aggregate[nodei] + 1
    ...
    for layerID in multiplex_structure:
        for nodei in multiplex_structure[layerID]:
            for neigh_of_i in multiplex_structure[layerID][nodei]:
                if degree_layers_multiplex[layerID][neigh_of_i] != 0:
                    # for every node adjacent to nodei in the current layer, we add the betweenness centrality of the node in the other layer - 1
                    fitness[nodei] += (2 * betweenness_centrality_multiplex[(layerID+1)%2][neigh_of_i]/((N-1)*(N-2)))
...
#### Ranking based on the generalisation of CI with Katz centrality in the case of duplex network
def compute_ranking_DCIz_katz_centrality(multiplex_structure, N, alpha=0.07):
    total_nodes = [i for i in range(N)]
    fitness = dict.fromkeys(total_nodes, 0)
    total_degree, participation_degree, dgr_part, degree_layers_multiplex = computing_total_degree_and_participation(multiplex_structure, N)
    ...
    for nodei in fitness:
        if katz_centrality_aggregate[nodei] != 0:
            # first term of the product of the generalised DCI
            numerator1 = (katz_centrality_multiplex[0][nodei] + 2) * (katz_centrality_multiplex[1][nodei] + 2)
            numerator2 = 3 * (katz_centrality_intersection[nodei] + 1) 
            numerator = numerator1 - numerator2 - 1
            denominator = katz_centrality_aggregate[nodei] + 1
...
#### Ranking based on the generalisation of CI with harmonic centrality in the case of duplex network
def compute_ranking_DCIz_harmonic_centrality(multiplex_structure, N):
    total_nodes = [i for i in range(N)]
    fitness = dict.fromkeys(total_nodes, 0)
    total_degree, participation_degree, dgr_part, degree_layers_multiplex = computing_total_degree_and_participation(multiplex_structure, N)
    ...
    for nodei in fitness:
        if harmonic_centrality_aggregate[nodei] != 0:
            # first term of the product of the generalised DCI
            numerator1 = (harmonic_centrality_multiplex[0][nodei] + 2) * (harmonic_centrality_multiplex[1][nodei] + 2)
            numerator2 = 3 * (harmonic_centrality_intersection[nodei] + 1)
            numerator = numerator1 - numerator2 - 1
            denominator = harmonic_centrality_aggregate[nodei] + 1
