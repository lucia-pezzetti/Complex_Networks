...
file_path_overlap_04 = '../output_files/output_file_04_third_trial.txt'
file_path_rewiring = '../output_files/output_file_rewiring.txt'
...
all_keys = list(outputs_overlap_02[0].keys())
markers = ['o', 's', '^', 'x', 'd', '*', '+', 'p']
...
ax1.set_title('Duplex with 0.00 overlap probability')
...
for k in range(len(all_keys)):
    ...
    ax2.scatter(np.linspace(-1, 1, 21), data, marker = markers[k], label=all_keys[k])
    ax2.plot(np.linspace(-1, 1, 21), data, linestyle='-', alpha=0.3, linewidth=1)
ax2.set_title('Duplex with 0.20 overlap probability')
...
ax3.set_title('Duplex with 0.40 overlap probability')
...
plt.tight_layout()
plt.show()
plt.savefig("../figures/interlyer_corr_vs_relative_size_critical_set.png")
...
plt.show()
plt.savefig("../figures/edge_overlap_vs_relative_size_critical_set.png")